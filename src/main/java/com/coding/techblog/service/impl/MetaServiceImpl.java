...
import com.coding.techblog.modal.Vo.MetaVoExample;
...
public class MetaServiceImpl implements IMetaService {
    ...
    public void saveMeta(String type, String name, Integer mid) {
        if (StringUtils.isNotBlank(type) && StringUtils.isNotBlank(name)) {
            ...
            MetaVo metas;
            if (metaVos.size() != 0) {
                throw new TipException("Đã tồn tại");
            } else {
                metas = new MetaVo();
                metas.setName(name);
                if (null != mid) {
    ...
    public void saveMetas(Integer cid, String names, String type) {
        if (null == cid) {
            throw new TipException("id tag không để trống");
        }
    ...
    private void saveOrUpdate(Integer cid, String name, String type) {
        ...
        if (metaVos.size() == 1) {
            ...
            throw new TipException("Truy vấn nhiều dữ liệu");
        } else {
            metas = new MetaVo();
            metas.setSlug(name);
            metas.setName(name);
